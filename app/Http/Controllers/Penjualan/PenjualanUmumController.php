...
class PenjualanUmumController extends Controller
{
    ...
    public function index(){
        ...
        return view('apps.penjualan.penjualan-umum.index',[ 'konsumens' => $konsumens, 'hargajuals' => $hargajuals ]);
    }
    ...
    public function store(Request $request){
        ...
        try{
            ...
            $invoices = DB::table('penjualans')->select(DB::raw('max(substr(kode,-4)) as nomor_max'))->where(DB::raw('MONTH(tanggal)'), $month)->where(DB::raw('YEAR(tanggal)'), $year)->where('kode','like','JSNI%')->get();

            $kodetransaksi = "JSNI".substr($year,-2).$month."-".Auth::id()."-".str_pad((int)$invoices[0]->nomor_max + 1,4,"0",STR_PAD_LEFT);
    ...
    public function requestdatapenjualan(Request $request){
        $penjualans = DB::table('penjualans')->where('status','Menunggu Pembayaran')->where('tipe_penjualan','Umum')->orderBy('created_at','desc')->get();
        ...
        if ($penjualans->count()){
            ...
            foreach($penjualans as $penjualan){
                ...
                $data .= '<div class="col-md-6 col-12" onclick="window.open(\'/penjualan/penjualan-umum/detil/' . $encrypt . '\',\'_self\')">'.
                    '<div class="card-detail">'.
                    '<div class="card-header">'.
                    '<h2 class="card-title col-md-12 text-center">'.$konsumen->nama.$catatannama.' - '.$penjualan->kategori.'</h2>'.
                    '</div>'.
    ...
    public function index_detil($param){
        ...
        return view('apps.penjualan.penjualan-umum.detil',[ 'penjualan' => $penjualan, 'produks' => $produks, 'akuns' => $akuns ]);
    }
            ...
            ->addColumn('action', function ($penjualans) {
                ...
                return '
                  <div class="fonticon-container">
                    <span class="fonticon-wrap '.$class3.' '.$class2.'" onclick="f_edittemp('.$penjualans->id.')"><i class="feather icon-edit" data-toggle="tooltip" title="Edit Timbangan"></i></span>
                    <span class="fonticon-wrap '.$class3.' '.$class.'" onclick="f_inputtara('.$penjualans->id.')"><i class="feather icon-truck" data-toggle="tooltip" title="Input Tara"></i></span>
                    <span class="fonticon-wrap '.$class3.'" onclick="f_deletetemp('.$penjualans->id.')"><i class="feather icon-trash" data-toggle="tooltip" title="Hapus Data"></i></span>
                  </div>
                ';
            })
            ->addIndexColumn()
    ...
    public function input_tara(Request $request){
        ...
        try{
            ...
            foreach($barangdetils as $barangdetil){
                ...
                $stokbaru = $stoklama - ($netto * $persentasepenggunaan);
...
//                'netto' => $netto,
    ...
    public function index_list(){
        ...
        return view('apps.penjualan.penjualan-umum.list-penjualan',[ 'konsumens' => $konsumens ]);
    }
    ...
    public function list_datapenjualan(Request $request){
        $penjualans = DB::table('penjualans')
            ...
            ->where(function($query) use ($request)
            {
                ...
                $query->where('penjualans.tipe_penjualan','=','Umum');
        ...
        return datatables()::of($penjualans)
            ->addColumn('action', function ($penjualans) {
                ...
                if(Auth::user()->status == "Staff" || Auth::user()->status == "Kasir" || Auth::user()->status == "Supervisor"){
                  $class = "hidden";
                  $class3 = "hidden";
                }
                ...
                return '
                  <div class="fonticon-container">
                    ...
                    <span class="fonticon-wrap '.$class2.'" onclick="window.open(\'/penjualan/penjualan-umum/edit-detil/' . $encrypt . '\',\'_blank\')"><i class="feather icon-edit" data-toggle="tooltip" title="Edit Data Penjualan"></i></span>
    ...
    public function index_edit_detil($param){
        ...
        return view('apps.penjualan.penjualan-umum.edit-detil',[ 'penjualan' => $penjualan, 'produks' => $produks, 'akuns' => $akuns ]);
    }
...
//                'netto' => $netto,
    ...
    public function print_data($param){
      ...
      $penjualan_details = DB::table('penjualan_details')
        ->join('barang_juals','penjualan_details.id_barang_juals','=','barang_juals.id')
        ->select('penjualan_details.*','barang_juals.nama as namabarang','barang_juals.kode as kodebarang','barang_juals.satuan as satuanbarang')
    ...
    public function hitung_ulang_stok(Request $request){


      DB::beginTransaction();
      try{

        $penjualans = DB::table('penjualan_details')->get();

        foreach($penjualans as $penjualan) {

          $count = DB::table('pemotongan_penjualans')->where('id_penjualan_details', $penjualan->id)->count();
          ...
          if ($count == 0) {
            ...
            foreach ($barangdetils as $barangdetil) {
              ...
              // insert table pemotongan
              DB::table('pemotongan_penjualans')->insert([
                'id_penjualan_details' => $penjualan->id,
                'id_barang_belis' => $barangdetil->id_barang_belis,
                'total' => $totalpenggunaan,
                "created_at" => $penjualan->updated_at,
