...
@section('title', 'Kartu Piutang Karyawan')
@section('page-title', 'Kartu Piutang Karyawan')
    ...
    <div class="col-md-12">
      <div class="row">
        ...
        <div class="col-sm-12 col-md-12 mt-1">
          <label for="client"> Filter Karyawan</label>
          <select class="form-control select" name="client" id="client">
            @foreach($clients as $karyawan)
              <option value="{{ $karyawan->id }}" data-foo="{{ $karyawan->kode }} - {{ $karyawan->keterangan }}">{{ $karyawan->nama }}</option>
            @endforeach
          </select>
        </div>

        <div class="col-sm-12 col-md-12 mt-2">
          <button type="button" id="btnsearch" name="btnsearch" onclick="f_loaddetil()" class="btn bg-gradient-info waves-effect waves-light"> <i class="feather icon-list" style="margin-right:8px;"></i> Lihat Data Piutang </button>
        </div>
...
@section('page-script')
  ...
  <script>
    ...
    $(document).ready(function() {
      ...
      $("#tanggalmulai").pickadate('picker').set('select',new Date());
      $("#tanggalselesai").pickadate('picker').set('select',new Date());
    ...
    function f_loaddetil(){
        ...
        if(tanggalmulai == "" || tanggalselesai == ""){
            alert('Pilih tanggal untuk melihat Laporan Piutang Karyawan');
            return;
        }

        let link = "/report/kartu-piutang-karyawan/list-detil" +
