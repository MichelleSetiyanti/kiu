...
<head>
  ...
  <title> Laporan Penjualan Detil </title>
        ...
        <div class="card-body p-0">
          <div class="invoice-container">
            <div class="invoice-header">
              ...
              <h2 style="color:#232F33;width:100%;text-align:center;font-family:'Poppins', sans-serif;"> LAPORAN PENJUALAN DETIL </h2>
                  ...
                  <div class="table-responsive">
                    <table class="table custom-table m-0">
                      ...
                      <tr>
                        ...
                        <th style="text-align:center;">Kode INV</th>
                      ...
                      @foreach($penjualans ?? '' as $penjualan)
                        <tr>
                          ...
                          <td style="text-align:center" >{{$penjualan->kode}}</td>
                          <td style="text-align:center" >{{$penjualan->kode_inv}}</td>
                          <td style="text-align:center" >{{$penjualan->kode_sj}}</td>
                          <td style="padding-left:10px;text-align:center"> {{ Carbon\Carbon::createFromFormat('Y-m-d', $penjualan->tanggal)->isoFormat('D MMMM Y') }} </td>
                          <td style="text-align:center" >{{$penjualan->namakonsumen}}</td>
                          <td style="text-align:center" >{{$penjualan->pembayaran}}</td>
                          <td style="text-align:center" >{{$penjualan->keterangan}}</td>
                          <td style="text-align:center" >{{ number_format($penjualan->grandtotal,0,',','.') }}</td>
                        </tr>
                        <tr>
                          <td></td>
                          <td colspan="9">
                            <table class="table custom-table m-0">
                              ...
                              <tr>
                                <th style="color: black !important;">Nama Barang</th>
                                <th style="color: black !important;">Qty</th>
                                <th style="color: black !important;">Harga</th>
                                <th style="color: black !important;">Diskon</th>
                                <th style="color: black !important;">Diskon Paket</th>
                                <th style="color: black !important;">Diskon Extra</th>
                                <th style="color: black !important;">Subtotal</th>
                                <th style="color: black !important;">Catatan</th>
                              </tr>
                              </thead>
                              <tbody>
                              @php
                              ...
                              foreach($penjualan_details as $penjualandetail){
                                ...
                                <tr>
                                  ...
                                  <td> {{ number_format($penjualandetail->total_jual,0,',','.') }} {{ strtoupper($penjualandetail->satuanbarang) }} </td>
                                  <td> {{ number_format($penjualandetail->harga,0,',','.') }} </td>
                                  <td> {{ number_format($penjualandetail->diskon,0,',','.') }} </td>
                                  <td> {{ number_format($penjualandetail->diskon_paket,0,',','.') }} </td>
                                  <td> {{ number_format($penjualandetail->diskon_extra,0,',','.') }} </td>
...
<script>
  ...
  function f_simpan(){
    ...
    html2canvas($(".html-content")[0],{scale:4}).then(function (canvas) {
      ...
      pdf.save("Laporan Penjualan Detil.pdf");
      $(".no-print").show();
    });
  }
</script>
