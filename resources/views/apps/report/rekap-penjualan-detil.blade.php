...
<head>
  ...
  <title> Laporan Rekap Penjualan Detil </title>
        ...
        <div class="card-body p-0">
          <div class="invoice-container">
            <div class="invoice-header">
              ...
              <h2 style="color:#232F33;width:100%;text-align:center;font-family:'Poppins', sans-serif;"> LAPORAN REKAP PENJUALAN </h2>
              ...
              <h5 style="color:#e31515;width:100%;text-align:center;font-family:'Poppins', sans-serif;">
                @if($request->client != "All")
                  @php
                    $konsumen = DB::table('konsumens')->where('id',$request->client)->first();
                  @endphp
                  ...
                  <div class="table-responsive">
                    <table class="table custom-table m-0">
                      ...
                      @foreach($barangs ?? '' as $barang)
                        @php
                          $sums = DB::table('penjualan_details')
                          ->join('penjualans','penjualan_details.id_penjualans','=','penjualans.id')
                          ->select(DB::raw('sum(penjualan_details.total_jual) as totaljual'),DB::raw('sum(penjualan_details.subtotal) as subtotalfinal'))
                          ->where('penjualan_details.id_barangs',$barang->id)
                          ->whereBetween('penjualans.tanggal', [$request->tanggalmulai, $request->tanggalselesai])
                          ...
                          $sumstotaljual += $sums[0]->totaljual;
                          $sumssubtotal += $sums[0]->subtotalfinal;

                          if($sums[0]->totaljual > 0){
                          $x++;
                        @endphp
                        <tr>
                          ...
                          <td>{{$barang->kode}}</td>
                          <td>{{$barang->nama}}</td>
                          <td style="text-align:right;">{{ number_format($sums[0]->totaljual,0,',','.') }} {{ strtoupper($barang->satuan) }}</td>
                          <td style="text-align:right;">Rp {{ number_format($sums[0]->subtotalfinal,0,',','.') }}</td>
                        </tr>
                        @php
                          }
                        @endphp
                      ...
                      <tr>
                        ...
                        <td style="text-align:right;padding-right:10px;font-size:22px;font-weight:bold;">{{ number_format($sumstotaljual,0,',','.') }}</td>
                        <td style="text-align:right;padding-right:10px;font-size:22px;font-weight:bold;">Rp {{ number_format($sumssubtotal,0,',','.') }}</td>
                      </tr>
                      </tbody>
                    </table>
                  </div>
