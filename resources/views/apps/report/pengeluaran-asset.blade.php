...
@section('title', 'Laporan Pengeluaran Operasional Asset')
@section('page-title', 'Laporan Pengeluaran Operasional Asset')
  ...
  <section id="data-list-view" class="data-list-view-header card" style="padding:25px 15px;">
      ...
      <div class="row">
        ...
        <div class="col-sm-12 col-md-6 mt-1">
          ...
          <select class="form-control select" name="kategori" id="kategori" required>
            <option value="All" data-foo="" selected> Semua Kategori </option>
            @foreach($kategoris as $kategori)
              <option value="{{ $kategori->id}}" data-foo="">{{ $kategori->nama }}</option>
            @endforeach
          </select>
        </div>
        ...
        <div class="col-sm-12 col-md-6 mt-1">
          ...
          <select class="form-control select" name="asset" id="asset" required>
            <option value="All" data-foo="" selected> Semua Asset </option>
            @foreach($assets as $asset)
              <option data-foo="{{ $asset->kode }}" value="{{ $asset->id }}">{{ $asset->nama }}</option>
            @endforeach
          </select>
        </div>

        <div class="col-sm-12 col-md-6 mt-1">
          <label for="akunkas">Akun Kas Pengeluaran</label>
          <select class="form-control select" name="akunkas" id="akunkas" required>
            <option value="All" data-foo="" selected> Semua Akun Kas Pengeluaran </option>
            @foreach($akunskas as $akun)
    ...
    <div class="table-responsive hidden mt-1" id="tabel">
      <table class="table data-list-view" style="border:none;width:100%;">
        ...
        <tfoot>
        <tr>
          <th colspan="9" style="font-size:18px;text-align:right;"></th>
...
@section('page-script')
  ...
  <script>
    ...
    function f_loadtable(){
      ...
      let asset = $("#asset").val();
      let akunkas = $("#akunkas").val();

      let link = "/report/pengeluaran-asset/list" +
      ...
      dataListView = $(".data-list-view").DataTable({
        ...
        columns: [
          ...
          {data: 'kategoripengeluaran', name: 'kategoripengeluaran'},
          {data: 'kolomakunkas', name: 'kolomakunkas'},
          {data: 'kolomakunbeban', name: 'kolomakunbeban'},
        ...
        "footerCallback": function ( row, data, start, end, display ) {
          ...
          $( api.column( 9 ).footer() ).html('Rp '+accounting.formatNumber(Total1, 0, ".", ","));
        },
      });
    }
