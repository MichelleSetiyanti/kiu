...
$temp = '';
if ($nilai < 12) { $temp=' ' . $huruf[$nilai]; } elseif ($nilai < 20) { $temp=penyebut($nilai - 10) . ' belas' ; }
    elseif ($nilai < 100) { $temp=penyebut($nilai / 10) . ' puluh' . penyebut($nilai % 10); } elseif ($nilai < 200) {
    $temp=' seratus' . penyebut($nilai - 100); } elseif ($nilai < 1000) { $temp=penyebut($nilai / 100) . ' ratus' .
    penyebut($nilai % 100); } elseif ($nilai < 2000) { $temp=' seribu' . penyebut($nilai - 1000); } elseif ($nilai <
    1000000) { $temp=penyebut($nilai / 1000) . ' ribu' . penyebut($nilai % 1000); } elseif ($nilai < 1000000000) {
    $temp=penyebut($nilai / 1000000) . ' juta' . penyebut($nilai % 1000000); } elseif ($nilai < 1000000000000) {
    $temp=penyebut($nilai / 1000000000) . ' milyar' . penyebut(fmod($nilai, 1000000000)); } elseif ($nilai <
    1000000000000000) { $temp=penyebut($nilai / 1000000000000) . ' trilyun' . penyebut(fmod($nilai, 1000000000000)); }
    return $temp; } function terbilang($nilai) { if ($nilai < 0) { $hasil='minus ' . trim(penyebut($nilai)); } else {
    $hasil=trim(penyebut($nilai)); } return $hasil . ' Rupiah' ; } use Illuminate\Support\Facades\DB; @endphp <!doctype
    html>
                                        ...
                                        <div class="col-lg-12 col-md-12 col-sm-12">
                                            <div class="table-responsive">
                                                <table class="table custom-table m-0">
                                                    <thead>
                                                        @php
                                                        $this_month = Carbon\Carbon::createFromFormat('m-Y',
                                                        $request->tanggalmulai);

                                                        $start_month = Carbon\Carbon::createFromFormat('m-Y',
                                                    ...
                                                    <tbody>
                                                        ...
                                                        @foreach (($request->pelunasan == 'Lunas' ? $lunas : $konsumens)
                                                        ?? [] as $konsumen)
                                                        <tr
                                                            ...
                                                            @for ($x = 0; $x <= $diff; $x++) @php
                                                                $tanggalmulai=Carbon\Carbon::createFromFormat('m-Y',
                                                                $request->tanggalmulai)
                                                                ->addMonths($x)
                                                                ->isoFormat('Y-MM') . '-01 00:00:00';
                                                                $tanggalselesai =
                                                                Carbon\Carbon::createFromFormat('m-Y',
                                                                $request->tanggalmulai)
                                                                ->addMonths($x)
                                                                ...
                                                                ->join('penjualans', 'bayar_piutangs.id_penjualans',
                                                                '=', 'penjualans.id')
                                                                ->where('penjualans.pajak', '>', 0)
                                                                ->join('konsumens', 'penjualans.id_konsumens', '=',
                                                                'konsumens.id')
                                                                ->select(DB::raw('sum(bayar_piutangs.nominal) as
                                                                totaljual'))
                                                                ->where('penjualans.id_konsumens', $konsumen->id)
                                                                ->whereBetween('bayar_piutangs.created_at',
                                                                ...
                                                                ->join('penjualans', 'bayar_piutangs.id_penjualans',
                                                                '=', 'penjualans.id')
                                                                ->where('penjualans.pajak', '<=', 0) ->join('konsumens',
                                                                    'penjualans.id_konsumens', '=', 'konsumens.id')
                                                                    ->select(DB::raw('sum(bayar_piutangs.nominal) as
                                                                    totaljual'))
                                                                    ->where('penjualans.id_konsumens', $konsumen->id)
                                                                    ->whereBetween('bayar_piutangs.created_at',
                                                                    ...
                                                                    ->where('penjualans.pajak', '<=', 0) ->where('sisa',
                                                                        '>', 0)
                                                                        ->join('konsumens', 'penjualans.id_konsumens',
                                                                        '=', 'konsumens.id')
                                                                        ->where('penjualans.id_konsumens',
                                                                        $konsumen->id)
                                                                        ->where('penjualans.pembayaran', '=', 'Credit')
                                                                        ->where('penjualans.status', '=', 'Selesai')
                                                                        ->select(DB::raw('sum(penjualans.sisa) as
                                                                        totaljual'))
                                                        ...
                                                        var sumblm = "{{ $sumblm }}";
                                                        if (sumblm == "0") {
                                                            document.getElementById('{{ $request->pelunasan == '
                                                                    Lunas ' ? '
                                                                    Lunas ' : '
                                                                    konsumen ' }}{{ $konsumen->id }}')
                                                                .style.display = "none";
                                                        }
                                                        if (sumkonsumen == "0") {
                                                            document.getElementById('{{ $request->pelunasan == '
                                                        ...
                                                        @endforeach
                                                        <tr @if ($request->client != 'All') style="display:none;"
                                                            @endif>
                                                            <td
                                                            ...
                                                            @for ($x = 0; $x <= $diff; $x++) @php
                                                                ...
                                                                ->where('penjualans.pajak', '<=', 0) ->join('konsumens',
                                                                    ...
                                                                    ->where('penjualans.pajak', '<=', 0) ->where('sisa',
                                                                        ...
                                                                        <td
                                                                            style="text-align:right;padding-right:10px;font-size:22px;font-weight:bold;">
                                                                            {{ number_format($sumsgrandtotal, 0, ',', '.') }}
                                                                        </td>
                                                                        @else
                                                                        <td
                                                                            style="text-align:right;padding-right:10px;font-size:22px;font-weight:bold;">
