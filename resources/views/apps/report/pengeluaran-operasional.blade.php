...
@section('page-title', 'Laporan Pengeluaran Operasional')
  ...
  <section id="data-list-view" class="data-list-view-header card" style="padding:25px 15px;">
    <div class="col-md-12">
      <div class="row">
        ...
        <div class="col-sm-12 col-md-6 mt-1">
          <label for="akunbeban">Akun Beban Pengeluaran</label>
          <select class="form-control select" name="akunbeban" id="akunbeban" required>
            <option value="All" data-foo="" selected> Semua Akun Beban Pengeluaran </option>
            @foreach($akunsbeban as $akun)
              <option data-foo="{{ $akun->kategori }}" value="{{ $akun->kode_akun }}">{{ $akun->kode_akun }} - {{ $akun->nama }}</option>
            @endforeach
          </select>
        </div>
...
@section('page-script')
  ...
  <script>
    ...
    function f_loadtable(){
      ...
      let akunbeban = $("#akunbeban").val();
      let divisi = $("#divisi").val();

      let link = "/report/pengeluaran-operasional/list" +
